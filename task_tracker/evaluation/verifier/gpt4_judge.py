...
class Judge:
    ...
    def extract_answer(self, raw_answer):
        ...
        try:
            final_assessment = (
                raw_answer.split("Final Assessment:")[-1].strip().replace(".", "")
            )
    ...
    def process_item(self, data_item, primary_ai_response):
        ...
        verifier_full_output, verifier_final_assessment = self.get_judge_response(
            primary_ai_response, user_task, text, extracted_task
        )
